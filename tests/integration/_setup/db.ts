...
export async function cleanupByPrefix(conn: Connection, prefix: string) {
  ...
  // by a repeat invocation. Order matters: children before parents because the
  // legacy schema declares FK constraints (payments.user_id → users.id, etc.).
  ...
  // billing_cycles columns vary across legacy snapshots; match by any of the
  // prefixable identifiers we control in fixtures.
  await conn.query(
    `DELETE FROM billing_cycles
     WHERE user_id LIKE ? OR customer_key LIKE ? OR billing_key LIKE ?`,
